...
namespace dota {
    ...
    class bitstream {
        public:
            ...
            /** Reads a bit angle */
            float nReadAngle(uint32_t n);
            
            /** Reads a normalized float vector */
            std::vector<float> nRead3BitNormal();
            
            /** Reads a normalized float */
            //float nReadNormal();

            /** Reads a little-endian uint16. */
            uint16_t nReadLeUInt16();
            
            /** Reads a little-endian uint32. */
            uint32_t nReadLeUInt32();

            /** Reads a little-endian uint64. */
            uint64_t nReadLeUInt64();
            
            /** Reads a big-endian uint16. */
            uint16_t nReadBeUInt16();
            
            /** Reads a big-endian uint32. */
            uint32_t nReadBeUInt32();

            /** Reads a big-endian uint64. */
            uint64_t nReadBeUInt64();
            
            /**
             * Reads a bit variable sized uint32_t from the stream.
             *
